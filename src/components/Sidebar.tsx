              ...
              <Link
                ...
                className={`flex items-center gap-3 px-3 py-2 rounded-lg text-sm font-medium transition-colors ${
                  isActive
                    ...
                    : "text-gray-600 hover:bg-indigo-100 hover:text-indigo-900"
                }`}
              >
                {item.icon}
                {item.label}
...
export function Sidebar() {
  ...
  return (
    <aside className="w-60 border-r border-indigo-100 bg-indigo-50 flex flex-col shrink-0">
      <nav className="flex-1 overflow-y-auto p-4 space-y-6">
        <NavSection title="Main" items={MAIN_NAV} pathname={pathname} />
        <NavSection title="Embed Demos" items={EMBED_NAV} pathname={pathname} />
      </nav>
